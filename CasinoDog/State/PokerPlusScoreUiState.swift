...
final class PokerPlusScoreUiState: ObservableObject, JSONSerializable {
    
    @Published var idToScroll: String = ""
    @Published var showMyScore: Bool = false
    
    let items: [PokerPlusScoreUiItem]
    
    init(items: [PokerPlusScoreUiItem]) {
        self.items = items
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        items = try container.decode([PokerPlusScoreUiItem].self, forKey: .items)
    }
...
final class PokerPlusScoreUiItem: ObservableObject, JSONSerializable  {
    let sort: Int
    let isMyScore: Bool
    let iconUrl: String
    let nickname: String
