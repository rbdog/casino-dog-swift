...
enum PartycakeExitRoom: HTTPAPI {
    ...
    static func run(request: Request) -> Response {
        ...
        case .bot:
            // botを停止
            center.partycakeServer.state.activeBots.first(where: {$0.userId == keycard.user_id})?.unlistenSpeaker()
            center.partycakeServer.state.activeBots.removeAll(where: {$0.userId == keycard.user_id})
            break
        }
        ...
        center.partycakeServer.speaker.speak(to: userIds, announce: announce)
        ...
        if !otherKeycards.contains(where: {$0.user_type == UserType.human.rawValue}) {
            ...
            for otherKeycard in otherKeycards {
                if otherKeycard.user_type == UserType.bot.rawValue {
                    // WebSocket停止
                    center.partycakeServer.state.activeBots.first(where: {$0.userId == otherKeycard.user_id})?.unlistenSpeaker()
                    center.partycakeServer.state.activeBots.removeAll(where: {$0.userId == otherKeycard.user_id})
                }
            }
            // 関連するキーカードを全て削除
            ...
            PartycakeStateRepositoryImpl().saveState(newState)
            
            // 新しいロボをActivate
            center.partycakeServer.activateBots(keycards: [newKeycard])
            // ベットを開始させる
            center.partycakeServer.state.activeBots
                .first(where: {$0.userId == newKeycard.user_id})?
                .startBet(at: PartycakeSeat(rawValue: newKeycard.seat)!, in: newState)
            
            let enterAnnounce = PartycakeAnnounce(
            ...
            center.partycakeServer.speaker.speak(to: otherKeycards.map{$0.user_id} + [newKeycard.user_id], announce: enterAnnounce)
