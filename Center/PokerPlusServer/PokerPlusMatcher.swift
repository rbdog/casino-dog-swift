...
class MatchingUser {
    ...
    func startTimer() {
        center.pokerPlusServer.matcher.onUserChangeMaxBotCount()
...
/// マッチメイキングする
class PokerPlusMatcher {
    
    let userRepository: UserRepository
    let stateRepository: PokerPlusStateRepository
    let keycardRepository: KeycardRealmRepository
    
    init(userRepository: UserRepository,
         stateRepository: PokerPlusStateRepository,
    ...
    func entryUser(id: String) {
        ...
        center.pokerPlusServer.state.matchingUsers.append(matchingUser)
    ...
    func onUserChangeMaxBotCount() {
        ...
        for botCount in 0...3 {
            ...
            availableUsers = center.pokerPlusServer.state.matchingUsers.filter({$0.maxBotCount >= botCount})
    ...
    func didCpmpleteAvailableUsers(users: [MatchingUser]) {
        ...
        for user in users{
            ...
            user.timer = nil
            center.pokerPlusServer.state.matchingUsers.removeAll(where: {$0.userId == user.userId})
        }
        // プレイコストを支払ったユーザーたち
        let willPayUsers = userRepository.read(whereIDs: users.map{$0.userId})
        let payedUsers = usersPayPlayCost(users: willPayUsers)
        // State を作成
        let state = PokerPlusRule.initialState
        // キーカード配列
        ...
        case 1:
            ...
            let keycard = KeycardRealm(
                ...
                stateID: state.id,
                seat: PokerPlusSeat.s3.rawValue
            )
            keycards = [keycard]
            
        case _ where count > 1 && count < 4:
            // 存在する分だけでキーカードを作成
            let sides = PokerPlusRule.initialSides
        ...
        // ボットを起動
        center.pokerPlusServer.activateBots(keycards: keycards)
        ...
        for keycardRealm in keycards {
            ...
            // WebSocket
            center.pokerPlusServer.speaker.speak(to: [keycard.user_id], announce: announce)
        }
    }
